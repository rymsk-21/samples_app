...
#特定のパスにリクエストした際、適切なレスポンスが返ってくるかテストしている。
# expectで始まる部分は「エクスペクテーション」といい、実際に検証しているところ。
# responseという変数にリクエストに対するレスポンスが入る。
# responses.bodyの検証
#　include マッチャで、タイトルとして指定の文字列が表示されているかを検証している。
